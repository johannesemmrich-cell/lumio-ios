...
struct LumioWidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> LumioWidgetEntry {
        LumioWidgetEntry.placeholder
    }

    func getSnapshot(in context: Context, completion: @escaping (LumioWidgetEntry) -> Void) {
        completion(LumioWidgetEntry.placeholder)
    }

    func getTimeline(in context: Context, completion: @escaping @Sendable (Timeline<LumioWidgetEntry>) -> Void) {
        Task.detached {
    ...
    private func fetchEntry() async -> LumioWidgetEntry {
        ...
        return LumioWidgetEntry(date: Date(), events: Array(events))
    }
}
...
struct LumioWidgetEntry: TimelineEntry {
    let date: Date
    let events: [WidgetEvent]

    static let placeholder = LumioWidgetEntry(
        date: Date(),
        events: [
...
struct LumioTodayWidget: Widget {
    let kind = "LumioTodayWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: LumioWidgetProvider()) { entry in
            LumioWidgetView(entry: entry)
...
struct LumioWidgetView: View {
    let entry: LumioWidgetEntry
